...
namespace red {
	...
	const auto DefaultWindowFlags =
			SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN | SDL_WINDOW_MOUSE_FOCUS |
	...
	var window::setTitle(object& self, var& args) {
		auto handle = (SDL_Window*)self.getPtr("handle");
		char* title = nullptr;
		if (args.getType() == typeString)
			title = (char*)args.getString();
		if (title != nullptr) {
	...
	var window::setFullscreen(object& self, var& args) {
		...
		if (handle)
			SDL_SetWindowFullscreen(
					handle, (fullscreen ? (desktop ? SDL_WINDOW_FULLSCREEN_DESKTOP
	...
	var window::create(object& self, var& args) {
		if (self.getType("handle") == typePtr)
			self("destroy");
		...
		uint32_t flags = DefaultWindowFlags |
		...
		SDL_Window* window =
				SDL_CreateWindow(title, windowX, windowY, width, height, flags);
	...
	var window::getConfig(object& self, var& args) {
		...
		for (auto i = self.begin(); i != self.end(); ++i) {
			auto allowedType = allowed.getType(i->first);
			if (allowedType != typeNull) {
				config.setVar(i->first, i->second);
			}
		}
		return var(config);
	}

	window::window() : object(proto) {
		(*this)("create");
	}

	window::window(object& config) : object(config) {
		setParent(&proto);
		(*this)("create");
	}
