...
namespace red {
	object entity::proto = object({{"id", var()},
																 {"name", "New Object"},
	...
	var entity::add(object& self, var args) {
		...
		if (args.isObject(&proto)) children->pushObject(args.getObject());
		if (args.isObject(&component::proto)) comps->pushObject(args.getObject());
	}

	var entity::remove(object& self, var args) {}

	var entity::initialize(object& self, var args) {
		if (self.getArray("components") == nullptr) self.setArray("components", array());
		if (self.getArray("children") == nullptr) self.setArray("children", array());
		if (self.getType("enabled") != typeBool) self.setBool("enabled", true);
	}
	...
	entity::entity() : object(proto) { this->callMethod("initialize"); }
