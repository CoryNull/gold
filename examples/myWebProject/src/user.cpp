...
namespace gg
{
	...
	gold::var user::create(obj data)
	{
		...
		if (invalidEmail(email, e))
			err.setString("email", e);
		if (invalidPassword(p, e))
			err.setString("password", e);
		if (invalidFirstName(fn, e))
			err.setString("firstName", e);
		if (invalidMidName(mn, e))
			err.setString("midName", e);
		if (invalidLastName(ln, e))
			err.setString("lastName", e);
		if (invalidCompany(com, e))
			err.setString("company", e);
		if (invalidPhone(ph, e))
			err.setString("phone", e);
		if (invalidGender(ge, e))
			err.setString("gender", e);
		if (invalidBirthday(bi, e))
			err.setString("birthday", e);
		if (invalidAddress1(a1, e))
			err.setString("address1", e);
		if (invalidAddress2(a2, e))
			err.setString("address2", e);
		if (invalidCountry(con, e))
			err.setString("country", e);
		if (invalidCity(city, e))
			err.setString("city", e);
		if (invalidZone(zone, e))
			err.setString("zone", e);
		if (invalidZip(zip, e))
			err.setString("zip", e);
		if (invalidIcon(icon, e))
			err.setString("icon", e);
